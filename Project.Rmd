...
2. We now implement the full model with cross validation. We have decided to use 10-fold cross validation, although only three repetitions were used for computational reasons. Fuerthermore, the total number of observations in the training set had to be reduced to 5000 observations.  
...
Now we run the model. We can observe that even when restricting to 5000 out of the 19622 available observations it is really slow.

```{r}
ptm <- proc.time()
modFitFull <- train(classe ~., data=training[sample(dim(training)[1], 5000), ],  trControl = fitControl, method="rf", prox=TRUE)
modFitFull
proc.time() - ptm
```
